...
module.exports = function (browser) {
  ...
  if (!cache[name]) {
    ...
    cache[name] = Q(_.clone(browser)).then(function (current) {
      ...
      var pathQueries = [].concat(current.pathQuery);
      var args = {cwd: current.cwd || '.'};
      var promises = _.map(pathQueries, function(pathQuery) {
          return Q.nfcall(exec, pathQuery, args)
      });

      // Run the pathQuery to see if we can find it somewhere else
      return Q.any(promises).then(function (stdout) {
        var path = utils.getStdout(stdout);
        // FIXME debug information is wrong, what is the current.pathQuery from the fulfilled prompise?
